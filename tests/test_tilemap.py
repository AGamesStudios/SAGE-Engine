...
def test_one_way_from_tileset(tmp_path):
    tmx = tmp_path / "map.tmx"
    tmx.write_text(
        """<?xml version='1.0' encoding='UTF-8'?>
<map width='1' height='1' tilewidth='32' tileheight='32'>
  <tileset firstgid='1'>
    <tile id='0'>
      <properties><property name='one_way' value='true'/></properties>
    </tile>
  </tileset>
  <layer name='main' width='1' height='1'>
    <data encoding='csv'>1</data>
  </layer>
</map>
"""
    ...
    world = physics.World()
    TileLayer.from_tmx(str(tmx), world=world)
    assert any(b.behaviour == "one_way" for b in world.bodies)


def test_grid_lines():
    layer = TileLayer(2, 1, [0, 0], tile_width=16, tile_height=16)
    lines = layer.grid_lines()
    assert (0, 0, 0, 16) in lines
    assert len(lines) == (2 + 1) + (1 + 1)
