...
def test_draw_simple_triangle():
    rt = GraphicRuntime()
    rt.init(64, 64)
    cam = Camera3D(position=Vector3(0, 0, -3), look_at=Vector3(0, 0, 0))
    cam_runtime.set_active_camera(cam)
    unit = Render3DUnit(rt)
...
def test_draw_without_camera_records_error():
    gfx.init(32, 32)
    gfx.begin_frame()
    cam_runtime.set_active_camera(None)
    mesh = Mesh3D([Vector3(-1, -1, 0), Vector3(1, -1, 0), Vector3(0, 1, 0)], [(0, 1, 2)])
    render_stats.reset_frame()
    draw_mesh(mesh, Matrix4.identity())
    gfx.flush_frame()
    assert render_stats.stats["camera_missing_count"] == 1
