...
`frame_ms` — длительность последнего кадра.
...
Модуль `graphics` предоставляет базовые классы `Mesh3D` и `Camera3D`.
Для отрисовки мешей используйте `Render3DUnit` из `render.units`. Он
проецирует треугольники на экран, выполняет depth‑test и увеличивает
`render.stats["triangles_drawn"]`.
