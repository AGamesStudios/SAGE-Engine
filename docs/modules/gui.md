...
Если тема не загружена явно, `GUIManager` применяет встроенную тему по умолчанию. Она задаёт стандартный шрифт `resources/fonts/default.ttf`, белый цвет текста и прозрачный фон. Все новые виджеты получают эти значения автоматически. Шрифт основан на **IBM Plex Sans** и распространяется под лицензией Apache 2.0. Файл можно заменить своим, сохранив имя `default.ttf`.
