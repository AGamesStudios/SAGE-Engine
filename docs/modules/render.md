...
Редактор SAGE Studio получает кадр предпросмотра через `core.get("render").draw_preview(scene_id)`.
Полученное изображение выводится в Canvas и обновляется по таймеру.
