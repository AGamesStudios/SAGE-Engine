...
Буфер также пересоздаётся мгновенно при получении события
`"window_resized"` из модуля `window`. `GraphicRuntime` подписан на это событие
и вызывает `realloc_buffer(width, height)` без ожидания следующего кадра.
