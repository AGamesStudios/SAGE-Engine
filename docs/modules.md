...
В движке каждый модуль отвечает за свою подсистему. Список подключаемых модулей задаётся ключом `boot_modules` в `engine.sagecfg`.
...
## 🔹 blueprint
Позволяет загружать и валидировать чертежи объектов. Поддерживает миграции схем без версий.
...
## 🔹 gui
Полноценная система интерфейсов с виджетами, компоновкой, стилями и событиями. Не требует сторонних библиотек и работает поверх `gfx`.
