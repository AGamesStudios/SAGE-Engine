...
### 🔄 Автозагрузка `libsagegfx`

При запуске движка будет автоматически выполнен поиск нативной библиотеки `libsagegfx.dll` / `.so` / `.dylib`:

- Не требует ручной установки переменных
- Можно положить библиотеку в `native/`, `lib/`, `bin/` или `build/`
- В случае отсутствия будет использован Software Renderer
