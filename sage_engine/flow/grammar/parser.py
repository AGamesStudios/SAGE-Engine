...
def _translate_ru(line: str) -> str:
    patterns = [
        ...
        (r"^конец$", r""),
        (r'^если\s+нажата\s+"([^"]+)"\s+тогда$', r'if Input.is_down("\1"):'),
        (r'^если\s+зажата\s+"([^"]+)"\s+тогда$', r'if Input.is_pressed("\1"):'),
        (r'^если\s+отпущена\s+"([^"]+)"\s+тогда$', r'if Input.is_up("\1"):'),
        (r'^если\s+(?:клавиша_нажата|нажата_клавиша)\s+"([^"]+)"\s+тогда$', r'if Input.is_pressed("\1"):'),
        (r"^если\s+(.+)\s+тогда$", r"if \1:"),
        (r"^глобал\s+(\w+)$", r"global \1"),
        (r"^лог\s+\"([^\"]+)\"$", r"log(\"\1\")"),
        (r"^иначе$", r"else:"),
        ...
        (r"увеличить\s+(\w+)\s+на\s+([\w\d]+)", r"\1 += \2"),
...
def _translate_en(line: str) -> str:
    patterns = [
        ...
        (r"^end$", r""),
        (r'^if\s+pressed\s+"([^"]+)"\s+then$', r'if Input.is_down("\1"):'),
        (r'^if\s+held\s+"([^"]+)"\s+then$', r'if Input.is_pressed("\1"):'),
        (r'^if\s+released\s+"([^"]+)"\s+then$', r'if Input.is_up("\1"):'),
        (r'^if\s+(?:key_pressed|pressed_key)\s+"([^"]+)"\s+then$', r'if Input.is_pressed("\1"):'),
        (r"^if\s+(.+)\s+then$", r"if \1:"),
        (r"^global\s+(\w+)$", r"global \1"),
        (r"^log\s+\"([^\"]+)\"$", r"log(\"\1\")"),
        (r"^else$", r"else:"),
        ...
        (r"increase\s+(\w+)\s+by\s+([\w\d]+)", r"\1 += \2"),
        (r"subtract\s+(\w+)\s+by\s+([\w\d]+)", r"\1 -= \2"),
        (r"decrease\s+(\w+)\s+by\s+([\w\d]+)", r"\1 -= \2"),
