...
class Camera:
    ...
    def _basis(self):
        ...
        def n(v): import numpy as np; m=float(np.linalg.norm(v)); return v if m<1e-8 else v/m
    ...
    def handle_input(self, window, dt: float):
        if getattr(window,'mouse_captured',False):
            cur=window.get_cursor_pos()
            if self._last_cursor is not None:
                dx=float(cur[0]-self._last_cursor[0]); dy=float(cur[1]-self._last_cursor[1])
                self.rot[1]+=dx*self.look_speed; self.rot[0]+=dy*self.look_speed
                self.rot[0]=max(-89.9,min(89.9,float(self.rot[0])))
            self._last_cursor=cur
        else:
            self._last_cursor=window.get_cursor_pos()
        import numpy as np
        fwd,right,up=self._basis(); m=np.zeros(3,dtype=np.float32)
        if window.is_key_down(glfw.KEY_W): m+=fwd
        if window.is_key_down(glfw.KEY_S): m-=fwd
        if window.is_key_down(glfw.KEY_D): m+=right
        if window.is_key_down(glfw.KEY_A): m-=right
        if window.is_key_down(glfw.KEY_SPACE): m+=np.array([0,1,0],dtype=np.float32)
        if window.is_key_down(glfw.KEY_LEFT_CONTROL) or window.is_key_down(glfw.KEY_LEFT_SHIFT): m+=np.array([0,-1,0],dtype=np.float32)
