...
class PaintWindow(QMainWindow):
    ...
    def _init_toolbar(self) -> None:
        ...
        color_action = QAction(choose_icon('color.png', 'plugin.png'), "Color", self)
        ...
        toolbar.addSeparator()
        zoom_in = QAction(choose_icon('zoom_in.png', 'add.png'), "Zoom +", self)
        zoom_in.triggered.connect(lambda: self.canvas.zoom_at(
            self.canvas.rect().center(), 1.2
        ))
        toolbar.addAction(zoom_in)

        zoom_out = QAction(choose_icon('zoom_out.png', 'cut.png'), "Zoom -", self)
